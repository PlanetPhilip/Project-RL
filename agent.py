...
class QAgent:
    ...
    def __init__(self,
        ...
        states = {
            ...
            'price': {'low': np.min(env.price_values), 'high': np.max(env.price_values),
                      'bin_size': custom_bin_size['price']},
        ...
        self.all_state_bins = {
            state: np.linspace(states[state]['low'], states[state]['high'], states[state]['bin_size'] + 1) for state in
            states}
