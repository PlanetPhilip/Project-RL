...
def train(agent, agent_nr, path, small_reward, large_reward, learning_rate, n_episodes, state_choice, state_bin_size, use_rewardshaping, show_plot=False, save_plot=False):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        if '--mode' not in sys.argv or sys.argv[sys.argv.index('--mode') + 1] == 'train':
            mode, agent, agent_nr, path, small_reward, large_reward, learning_rate, n_episodes, state_choice, state_bin_size, use_rewardshaping = parse_train_arguments()
            locals()[mode](agent, agent_nr, path, small_reward, large_reward, learning_rate, n_episodes, state_choice, state_bin_size, use_rewardshaping)
        else:
        ...
        # Default behavior
        mode, agent, agent_nr, path, small_reward, large_reward, learning_rate, n_episodes, state_choice, state_bin_size, use_rewardshaping = (
            'train', 
            'QAgent', 
            '1', 
            '', 
            ...
            0.01, 
            10, 
            ['storage_level', 'price', 'hour', 'Day_of_Week', 'Season'],
            [10, 10, 24, 7, 4],
            True
        )
        locals()[mode](agent, agent_nr, path, small_reward, large_reward, learning_rate, n_episodes, state_choice, state_bin_size, use_rewardshaping)
